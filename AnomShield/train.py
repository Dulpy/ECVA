...
sys.path.append('../')
from AnomShield import conversation as conversation_lib
from AnomShield.model import *
from AnomShield.constants import NUM_FRAMES, IGNORE_INDEX, MMODAL_TOKEN_INDEX, DEFAULT_MMODAL_TOKEN, DEFAULT_MMODAL_START_TOKEN, DEFAULT_MMODAL_END_TOKEN
from AnomShield.mm_utils import tokenizer_MMODAL_token, tokenizer_image_token, expand2square, process_video, process_image
from AnomShield.AnomShield_trainer import (
    AnomShieldTrainer,
...
class ModelArguments:
    ...
    version: Optional[str] = field(default="mistral", metadata={"help": "Version of the conversation template."})
    freeze_backbone: bool = field(default=False, metadata={"help": "Whether to freeze the LLM backbone."})
    # Connector Arguments
    mamba_depth: int = field(default=1)
    mlp_depth: int = field(default=2)
    mm_projector_type: Optional[str] = field(default='mamba_scan')
    tune_mm_mlp_adapter: bool = field(default=False)
    pretrain_mm_mlp_adapter: Optional[str] = field(default=None)
    # Vision tower Arguments
    vision_tower: Optional[str] = field(default='clip-vit-large-patch14')
...
class DataArguments:
    ...
    data_folder: Optional[str] = field(default='/datasetsvideosft/')
    # Loading Arguments
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        if 'mistral' in model_args.model_name_or_path.lower():
            ...
            model = AnomShieldMistralForCausalLM.from_pretrained(
    ...
    if training_args.lora_enable:
        ...
        for name, param in model.named_parameters():
            rank0_print(f"Parameter name: {name}, requires_grad: {param.requires_grad}")
    ...
    # select a Trainer
    trainer = AnomShieldTrainer(model=model, tokenizer=tokenizer, args=training_args, **data_module)
