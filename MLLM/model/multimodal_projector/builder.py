...
class MambaScan(nn.Module):
    def __init__(self, config, drop_path_rate=0.1,mamba_depth=2, mlp_depth=2, num_frames=16, num_patches=256):
        super().__init__()
        self.spatio_pos_emb = nn.Parameter(torch.zeros(1, num_patches, 1024))
        self.temporal_pos_emb = nn.Parameter(torch.zeros(1, num_frames, 1024))

        # self.spatio_pos = nn.Parameter(torch.zeros(1, 16, 1024))
        # self.temporal_pos = nn.Parameter(torch.zeros(1, 16, 1024))
        ...
        # 实验
        # self.mambaModel_w = MambaModel(config.mm_hidden_size, drop_path_rate=drop_path_rate, depth=mamba_depth)
    
    def forward(self, video_spatio_temporal_features, flag=False):
        if flag:
            b, seg, f, p, d = video_spatio_temporal_features.shape
        else:
            b, f, p, d = video_spatio_temporal_features.shape
        w, h = int(p**0.5), int(p**0.5)
        video_spatio_temporal_features = rearrange(video_spatio_temporal_features, 'b seg f (h w) d -> b (seg f) h w d',
                                                    h=h, w=w) if flag else rearrange(video_spatio_temporal_features, 'b f (h w) d -> b f h w d',
                                                    h=h, w=w)
        video_spatio_temporal_features = rearrange(video_spatio_temporal_features, 'b f h w d -> (b h w) f d', b=b, w=w,
                                                    f=f)
        # video_spatio_temporal_features = video_spatio_temporal_features + self.temporal_pos_emb
        x = rearrange(video_spatio_temporal_features, '(b h w) f d -> b (f h w) d', b=b, w=w, f=f)
        x = self.mambaModel_h(x)  # shape(bs,frames*width*height,feature_dim)
        # y = rearrange(video_spatio_temporal_features, '(b h w) f d -> b (f w h) d', b=b, w=w, f=f)
        # y = self.mambaModel_v(y)
        # y = rearrange(y, 'b (f w h) d -> b (f h w) d', b=b, w=w, f=f)
        # pooling and norm
        ...
        #                                            h=h, w=w) if flag else rearrange(video_spatio_temporal_features, 'b f (h w) d -> b f h w d',
        #                                            h=h, w=w)
        # video_spatio_temporal_features = rearrange(video_spatio_temporal_features, 'b f h w d -> (b f) (h w) d', b=b, w=w,
        #                                            f=f)
        # video_spatio_temporal_features = video_spatio_temporal_features + self.spatio_pos_emb
        # video_spatio_temporal_features = rearrange(video_spatio_temporal_features, '(b f) (h w) d -> (b h w) f d', b=b, w=w,
        #                                            f=f)
        # video_spatio_temporal_features = video_spatio_temporal_features + self.temporal_pos_emb
        # x = rearrange(video_spatio_temporal_features, '(b h w) f d -> b (f h w) d', b=b, w=w, f=f)
