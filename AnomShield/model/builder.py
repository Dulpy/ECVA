...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, **kwargs):
    ...
    if "mamba" in model_name.lower():
        ...
        if 'lora' in model_name.lower() or 'qlora' in model_name.lower():
            ...
            if 'mistral' in model_base.lower():
                model = AnomShieldMistralForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=lora_cfg_pretrained, **kwargs)
            else:
                model = AnomShieldMistralForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=lora_cfg_pretrained, **kwargs)
            ...
            if 'mistral' in model_base.lower():
                model = AnomShieldMistralForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=cfg_pretrained, **kwargs)
            else:
                model = AnomShieldMistralForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=cfg_pretrained, **kwargs)
            ...
            if 'mistral' in model_base.lower():
                ...
                model = AnomShieldMistralForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, **kwargs)
            else:
    ...
    if "mamba" in model_name.lower():
        ...
        # vision_tower.to(torch.float16).to('cuda:4')
